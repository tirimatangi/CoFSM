...
private:
    ...
    static constexpr unsigned char _enableCode = 0x4;
...
#endif
        std::cout << "Red   LED = " << (_ledStatus == Status::Off ? "Off" : "On") << '\n';
    }
...
static CoFSM::State redActiveState(FSM& fsm)
{
    RedLedControl ledControl;
    ...
    while (true)
    {
        ...
        {  // Keep the LED on for the given time in milliseconds.
            ledControl.set(RedLedControl::Status::On);
            ...
            ledControl.set(RedLedControl::Status::Off);
...
static CoFSM::State redIdleState(FSM& fsm)
{
    constexpr int iBlinkTimeMs = 250;   // Time of a single blink in milliseconds
