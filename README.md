...
Documentation will appear here in February 2023. Meanwhile, take a look at the examples.
...
/* Creates the states and sets the transition table like so:
   [ state ]  --- ToPongEvent ---> [ state ]
   [ Ping  ] <--- ToPingEvent ---  [ Pong  ]
*/
FSM& setup(FSM& fsm)
{
...
int main()
{
    ...
    fsm.setState("pingState").sendEvent(&e);
...
![Morse code with CoFSM (video)](examples/fsm-example-morse/fsm-morse.mp4)

![Morse code with CoFSM (state diagram)](examples/fsm-example-morse/fsm-morse.png)

## Example: Connect and separate FSMs in runtime
...
## Example: Configure an FSM programmatically and measure the transit speed
